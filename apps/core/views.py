...
def delete_commodities(request):
    try:
        ...
        ids = [int(i) for i in data.get("commodity_ids", [])]

        Commosities = CommodityMater.objects.filter(id__in=ids)

        for wg in Commosities:
            delete_with_related(wg)

        return JsonResponse({"success": True, "message": f"{Commosities.count()} Commodities and related data deleted."})
