...
@require_POST
def delete_waste_collector(request):
    try:
        data = json.loads(request.body.decode('utf-8'))
        ids = [int(i) for i in data.get("waste_collector_ids", [])]

        waste_groups = Collector.objects.filter(id__in=ids)

        for wg in waste_groups:
            delete_with_related(wg)

        return JsonResponse({"success": True, "message": f"{waste_groups.count()} waste Collector and related data deleted."})
