...
@require_POST
def delete_agreements(request):
    try:
        data = json.loads(request.body.decode('utf-8'))
        ids = [int(i) for i in data.get("agreements_ids", [])]

        Agreements = Agreement.objects.filter(id__in=ids)

        for wg in Agreements:
            delete_with_related(wg)

        return JsonResponse({"success": True, "message": f"{Agreements.count()} Agreements and related data deleted."})
