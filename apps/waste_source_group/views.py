...
def delete_with_related(instance):
    """
    Recursively delete all related objects before deleting this instance.
    """
    for rel in instance._meta.get_fields():
        # Check for reverse relations that are NOT many-to-many
        if (rel.one_to_many or rel.one_to_one) and rel.auto_created and not rel.concrete:
            ...
            if isinstance(related_manager, models.Model):
                ...
                delete_with_related(related_manager)
            else:
                # OneToMany relation (RelatedManager)
                for obj in related_manager.all():
                    delete_with_related(obj)
    instance.delete()
...
def delete_waste_source_master(request):
    try:
        ...
        ids = [int(i) for i in data.get("waste_source_master_ids", [])]

        source_groups = MasterSource.objects.filter(id__in=ids)

        for wg in source_groups:
            delete_with_related(wg)

        return JsonResponse({"success": True, "message": f"{source_groups.count()} waste groups and related data deleted."})
...
def delete_waste_group_master(request):
    try:
        data = json.loads(request.body.decode('utf-8'))
        ids = [int(i) for i in data.get("waste_group_master_ids", [])]

        waste_groups = WasteGeneratorGroup.objects.filter(id__in=ids)

        for wg in waste_groups:
            delete_with_related(wg)

        return JsonResponse({"success": True, "message": f"{waste_groups.count()} waste groups and related data deleted."})
